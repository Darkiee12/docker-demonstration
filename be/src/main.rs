...
use std::{net::IpAddr, path::Path, sync::Arc, vec};
...
async fn main() {
    dotenv().ok();
    let address: IpAddr = std::env::var("ROCKET_ADDRESS")
        .and_then(|x| x.parse().map_err(|_| std::env::VarError::NotPresent))
        .expect("IP environment variable is not set or is not a valid IP address");
    let port: u16 = std::env::var("ROCKET_PORT")
        .and_then(|x| x.parse().map_err(|_| std::env::VarError::NotPresent))
        .expect("PORT environment variable is not set or is not a valid number");

    let mut config = rocket::Config::default();
    config.address = address;
    config.port = port;
    let cors = CorsOptions::default()
        .allowed_origins(AllowedOrigins::some_regex(&[r"^http://localhost:\d+$"]))
...
fn detect(file: Form<TempFile<'_>>) -> String {
    println!("Received file at {}", Utc::now().to_rfc3339());
    let buf: Vec<u8> = std::fs::read(file.path().unwrap_or(Path::new(""))).unwrap_or(vec![]);
    let Ok(boxes) = detect_objects_on_image(buf) else{
        return "[]".to_string();
    };
    let output: Vec<Detection> = boxes
        .iter()
        .map(|x| Detection {
            x1: x.0,
            y1: x.1,
        ...
        .collect();
    println!("Send response at {}", Utc::now().to_rfc3339());
    let result = serde_json::to_string(&output).unwrap_or_default();
    // println!("{}", result);
    return result;
}
...
fn detect_objects_on_image(buf: Vec<u8>) -> Result<Vec<(f32, f32, f32, f32, &'static str, f32)>, Error> {
...
fn process_output(
    ...
    img_width: u32,
    img_height: u32,
) -> Result<Vec<(f32, f32, f32, f32, &'static str, f32)>, Error>{
